...
class TestBaseLLMClient(unittest.TestCase):
    ...
    def test_base_client_is_abstract(self):
        ...
        print("   [OK] BaseLLMClient is abstract as expected")
    ...
    def test_message_validation(self):
        ...
        print("   [OK] Message validation works correctly")
...
class TestLLMClientFactory(unittest.TestCase):
    ...
    def test_get_available_backends(self):
        ...
        print(f"   [OK] Found {len(backends)} local backends:")
    ...
    def test_create_ollama_client(self):
        ...
        try:
            ...
            print("   [OK] Ollama client created successfully")
            print(f"      Model: {client.model}")
            print(f"      Base URL: {client.base_url}")
        except Exception as e:
            print(f"   [SKIP] Ollama client creation skipped: {e}")
    ...
    def test_create_lmstudio_client(self):
        ...
        try:
            client = LLMClientFactory.create("lmstudio", model="qwen3.5-2b")

            self.assertIsInstance(client, LMStudioClient)
            self.assertEqual(client.model, "qwen3.5-2b")

            print("   [OK] LM Studio client created successfully")
            print(f"      Model: {client.model}")
            print(f"      Base URL: {client.base_url}")
        except Exception as e:
            print(f"   [SKIP] LM Studio client creation skipped: {e}")
    ...
    def test_create_invalid_backend(self):
        ...
        print("   [OK] Invalid backend error handled correctly")
    ...
    def test_check_backend_availability(self):
        ...
        icon = "[OK]" if ollama_available else "[NO]"
        ...
        icon = "[OK]" if lmstudio_available else "[NO]"
        print(f"   {icon} LM Studio: {status}")

        print("   [INFO] Availability check completed")
...
class TestMemAgentMultiBackend(unittest.TestCase):
    ...
    def test_mem_agent_default_ollama(self):
        ...
        try:
            ...
            print("   [OK] MemAgent created with Ollama backend")
        except Exception as e:
    ...
    def test_mem_agent_lmstudio(self):
        ...
        try:
            agent = MemAgent(
                model="qwen3.5-2b", backend="lmstudio", use_sql=False, check_connection=False
            )

            self.assertIsNotNone(agent.llm)
            self.assertIsInstance(agent.llm, LMStudioClient)
            print("   [OK] MemAgent created with LM Studio backend")
        except Exception as e:
            print(f"   [SKIP] Test skipped: {e}")
...
class TestOllamaClient(unittest.TestCase):
    ...
    def test_ollama_connection(self):
        ...
        if not self.available:
            print("   [SKIP] Ollama not available, skipping test")
            return
    ...
    def test_ollama_list_models(self):
        ...
        print(f"   [OK] Found {len(models)} Ollama models")
        if models:
...
class TestLMStudioClient(unittest.TestCase):
    ...
    def setUp(self):
        """Set up test client"""
        try:
            self.client = LMStudioClient(model="qwen3.5-2b")
            self.available = self.client.check_connection()
        except Exception:
            self.client = None
            self.available = False
    ...
    def test_lmstudio_connection(self):
        ...
        if not self.available:
            print("   [SKIP] LM Studio not available, skipping test")
            return
